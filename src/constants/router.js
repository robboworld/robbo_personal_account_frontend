export const LANDING_PAGE_ROUTE = '/'
export const HOME_PAGE_ROUTE = '/home'
export const LOGIN_PAGE_ROUTE = '/login'
export const REGISTER_PAGE_ROUTE = '/register'
export const PROFILE_PAGE_ROUTE = '/profile'
export const CUSTOMIZATION_PAGE_ROUTE = '/customization'
export const PEEK_PROFILE_PAGE = '/peekprofile'
export const PROJECT_PAGE_ROUTE = '/projects/:projectPageId'
export const PUBLIC_PROJECTS_ROUTE = '/projects/public'
export const MY_PROJECTS_ROUTE = '/myprojects'
export const SCRATCH_HUB_ROUTE = '/scratch'
export const MY_COURSES_ROUTE = '/mycourses'
export const COURSE_PAGE_ROUTE = '/courses/:coursePageId'
export const TEACHERS_PAGE_ROUTE = '/teachers'
export const CLIENTS_ROUTE = '/clients'
export const UNIT_ADMINS_ROUTE = '/unitAdmins'
export const ROBBO_UNITS_ROUTE = '/robboUnits'
export const ROBBO_GROUPS_ROUTE = '/robboGroups'
export const ROBBO_UNIT_PAGE_ROUTE = '/robboUnits/:robboUnitId'
export const ROBBO_UNIT_STUDENT_GROUPS_PAGE = '/robboUnits/:robboUnitId/groups'
export const ROBBO_GROUP_PAGE_ROUTE = '/robboUnits/:robboUnitId/groups/:robboGroupId'
export const OIDC_CALLBACK_ROUTE = '/auth/oidc/callback'
export const SEND_NOTIFICATION_ROUTE = '/send-notification'
export const USERS_ROUTE = '/users'
export const MY_LICENSES_ROUTE = '/licenses'
export const MY_SESSIONS_ROUTE = '/sessions'
export const ISSUE_LICENSE_ROUTE = '/licenses/issue'
export const LICENSES_CATALOG_ROUTE = '/licenses/buy'
export const PAYMENT_RECEIPT_ROUTE = '/licenses/receipt'
export const DEVICE_LINK_ROUTE = '/device/link'
export const TEACHER_CLASSES_ROUTE = '/teacher/classes'
export const TEACHER_CLASS_BOARD_ROUTE = '/teacher/classes/:classId'
export const STUDENT_CLASSES_ROUTE = '/myclasses'
export const JOIN_CLASS_ROUTE = '/join/:slug'
export const JOIN_CLASS_CODE_ROUTE = '/join'